...
class PassInsideView():
    ...
    def __init__(self, name, challenge_id, category, description, points, file,
                 flag, author):
        self.name = name
...
def index(request):
    ...
    for c in challenge:
        if c.category == 'Stegnography':
            s = PassInsideView(c.name, assignID(c.name), c.category,
                               c.description, c.points, c.file, c.flag,
                               c.author)
            challenge_info_stego_object.append(s)
        elif c.category == 'Reverse Engineering':
            re = PassInsideView(c.name, assignID(c.name), c.category,
                                c.description, c.points, c.file, c.flag,
                                c.author)
            challenge_info_re_object.append(re)
        elif c.category == 'Forensics':
            f = PassInsideView(c.name, assignID(c.name), c.category,
                               c.description, c.points, c.file, c.flag,
                               c.author)
            challenge_info_for_object.append(f)
        elif c.category == 'Pwning':
            p = PassInsideView(c.name, assignID(c.name), c.category,
                               c.description, c.points, c.file, c.flag,
                               c.author)
            challenge_info_pwn_object.append(p)
        elif c.category == 'Web':
            w = PassInsideView(c.name, assignID(c.name), c.category,
                               c.description, c.points, c.file, c.flag,
                               c.author)
            challenge_info_web_object.append(w)
        elif c.category == 'Cryptography':
            cy = PassInsideView(c.name, assignID(c.name), c.category,
                                c.description, c.points, c.file, c.flag,
                                c.author)
...
def flagsubmit(request):
    ...
    if flag == flag_submit and not request.user.is_superuser:
        fr = models.ChallengesSolvedBy(challenge_id=flag_submit_id,
                                       user_name=request.user, points=points)
        try:
            fc = models.ChallengesSolvedBy.objects.filter(
            ...
            if flag_submit_id in obs:
                ...
                fr.save()
                initial_points = accounts_models.Teams.objects.get(
                    teamname=request.user).points
                updated_points = initial_points + points
                accounts_models.Teams.objects.filter(
                    teamname=request.user).update(points=updated_points)
            ...
            initial_points = accounts_models.Teams.objects.get(
                teamname=request.user).points
            updated_points = initial_points + points
            accounts_models.Teams.objects.filter(teamname=request.user).update(
