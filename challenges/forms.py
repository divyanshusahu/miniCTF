...
class AddChallengeForm(forms.ModelForm):
    name = forms.CharField(max_length=250, label="Challenge Name *",
                           widget=forms.TextInput(
                               attrs={'placeholder': 'Challenge Name',
                                      'class': 'form-control'}))
    ...
    description = forms.CharField(max_length=1000, widget=forms.TextInput(
        attrs={'placeholder': 'Challenge Description', 'class': 'form-control',
               'required': 'false'}), label="Challenge Description *",
                                  required=False)
    points = forms.IntegerField(
        widget=forms.NumberInput(attrs={'class': 'form-control'}),
        label="Challenge Points *")
    file = forms.FileField(label="Challenge Files (if any)", required=False)
    flag = forms.CharField(max_length=500, label="Challenge Flag *",
                           widget=forms.TextInput(
                               attrs={'placeholder': 'Challenge Flag',
                                      'class': 'form-control'}))
    author = forms.CharField(max_length=250, label="Challenge Author *",
                             widget=forms.TextInput(
                                 attrs={'placeholder': 'Challenge Author',
                                        'class': 'form-control'}))
