    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            try:
                ...
                user = User.objects.create_user(username=teamname,
...
def profile(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            team = request.user
            models.Teams.objects.filter(teamname=team).update(job=job,
                                                              company=company)
            success = 1
...
def team_view(request):
    ...
    solved_challenges = ChallengesSolvedBy.objects.filter(
...
def every_team(request, pk):
    requested_team = pk
    try:
        requested_team_details = models.Teams.objects.get(
            teamname=requested_team)
        solved_team_challenges = ChallengesSolvedBy.objects.filter(
            user_name=requested_team)
        return render(request, 'team/team.html',
                      {'team_details': requested_team_details,
                       'solved_challenges': solved_team_challenges})
